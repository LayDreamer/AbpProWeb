...
const currentState = ref(LoginStateEnum.DOMAIN);
...
export function useLoginState() {
  ...
  function handleBackLogin() {
    setLoginState(LoginStateEnum.DOMAIN);
  }
...
export function useFormRules(formData?: Recordable) {
  ...
  const getDomainFormRule = computed(() => createRule(t('sys.login.accountPlaceDomainholder')));
  ...
  const getFormRules = computed((): { [k: string]: ValidationRule | ValidationRule[] } => {
    ...
    const domianFormRule = unref(getDomainFormRule);
    ...
    switch (unref(currentState)) {
      ...
      case LoginStateEnum.MOBILE:
        return mobileRule;
      case LoginStateEnum.DOMAIN:
        return {
          account: domianFormRule,
          password: passwordFormRule,
        };
      // login form rules
      default:
